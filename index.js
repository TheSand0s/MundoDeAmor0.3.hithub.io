document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleAcceptMission(e) {
        ...
        const templateParams = { user_name: currentUser.Nombre.charAt(0).toUpperCase() + currentUser.Nombre.slice(1), mission_title: missionTitle, magic_link: magicLink, to_email: currentUser.Nombre === 'nanita' ? sandyEmail : nanitaEmail };
        try {
            await emailjs.send('service_3w96w7w', 'template_vlr3k3d', templateParams);
            alert(`¡Has aceptado la misión "${missionTitle}"!\nSe ha enviado un correo a ${currentUser.Nombre === 'nanita' ? 'sandy' : 'nanita'} para que la apruebe.`);
        } catch (error) { console.error('Error al enviar el correo de misión:', error); alert('Misión aceptada, pero hubo un error al enviar la notificación.'); }
        e.target.disabled = true; e.target.textContent = 'Pendiente...';
    }
